...
. There are 2 options for writing a new operation. Optiona `a` is to write a device operation and option `b` is to write a composite operation
...
. Register the struct using `ttnn::register_operation`.

## What steps are needed to add TT-NN operation in Python?
. Take an existing registered C++ operation and add a Python binding for it using `ttnn::bind_registered_operation`.
