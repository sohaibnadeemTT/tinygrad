<a name="device_program_profiler"></a>
...
The commands to build and run the `full_buffer` example after following [Getting Started<Getting Started>](#Getting Started<Getting Started>):
...
. Setting `LOOP_COUNT` and `LOOP_SIZE` defines for the kernels
. Calling [DumpDeviceProfileResults<DumpDeviceProfileResults>](#DumpDeviceProfileResults<DumpDeviceProfileResults>) after the call to Finish to collect the device side profiling data
